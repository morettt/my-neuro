...
**2. 广场下载功能修复**
- 修复工具广场下载失败问题（参数命名不一致）
- 自动为缺少 `download_url` 的工具构建下载URL
- 兼容多种参数名：`download_url`、`tool_url`、`id`、`tool_id`、`name`

**3. 删除独立测试服务器**
- 删除 `market_download_api.py` - 独立广场下载API测试服务器
- 删除 `market_patch.js` - 浏览器控制台补丁脚本
...
|------|------|----------|
| ~~复选框反复跳动~~ | ✅ 已解决 | 禁用配置轮询 |
| ~~部分配置项无反应~~ | ✅ 已解决 | 修复前后端字段名映射 |
| ~~广场下载失败~~ | ✅ 已解决 | 统一参数命名，自动构建下载URL |
