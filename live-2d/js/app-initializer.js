...
class AppInitializer {
    ...
    async initializeToolManagers() {
        ...
        try {
            ...
            // 初始化博客生成器
            if (this.config.blog_generator?.enabled) {
                try {
                    const { blogGenerator } = require('../server-tools/blog_generator.js');
                    blogGenerator.initialize(this.config.blog_generator, this.config.llm);
                    global.blogGenerator = blogGenerator;
                    console.log('博客生成器初始化成功');
                    logToTerminal('info', '博客生成器初始化成功');
                } catch (error) {
                    console.error('博客生成器初始化失败:', error);
                    logToTerminal('error', `博客生成器初始化失败: ${error.message}`);
                }
            }
