...
class VoiceChatFacade {
    ...
    // ========== 新增：文件暂存与管理方法 ==========
    stageFileData(fileData) {
        this._pendingFileData.push(fileData);
        console.log(`[VoiceChat] 已暂存文件: ${fileData.fileName}。当前缓存: ${this._pendingFileData.length} 个文件。`);
        
        // 统计并返回各类文件数量
        const counts = this._pendingFileData.reduce((acc, file) => {
        ...
        return {
            ...
            docCount: counts.text || 0,
        };
    }

    getStagedFiles() {
        return this._pendingFileData;
    }

    clearStagedFiles() {
        this._pendingFileData = [];
        console.log('[VoiceChat] 文件缓存已清空。');
    }
